...
from neuroagent.autogenerated_types.thumbnail_generation import (
...
class PlotElectricalCellRecordingGetOneTool(BaseTool):
    ...
    async def arun(self) -> PlotElectricalCellRecordingGetOneOutput:
        ...
        electrical_recording = await self.metadata.httpx_client.get(
            url=self.metadata.entitycore_url.rstrip("/")
            + f"/electrical-cell-recording/{self.input_schema.entity_id}",
            headers=headers,
        )
        if electrical_recording.status_code != 200:
            raise ValueError(
                f"The Electrical Cell Recording endpoint returned a non 200 response code. Error: {electrical_recording.text}"
            )
        assets = electrical_recording.json()["assets"]

        # Get the nwb asset
        nwb_asset = next(
            (asset for asset in assets if asset["content_type"] == "application/nwb"),
            None,
        )
        if not nwb_asset:
            raise ValueError(
        ...
        query_params["asset_id"] = nwb_asset["id"]
