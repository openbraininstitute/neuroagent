...
from neuroagent.tools.autogenerated_types.bluenaas import (
    MEModelResponse,
    RetrieveNeuronModelApiBluenaasNeuronModelVirtualLabIdProjectIdModelIdGetResponse,
...
class MEModelGetOneTool(BaseTool):
    ...
    async def arun(self) -> MEModelGetOneToolOutput:
        ...
        # The input contains only path parameters => No autogen model for it
        ...
        if resp_json["type"] == "synaptome":
            return MEModelGetOneToolOutput(root=SynaptomeModelResponse(**resp_json))
        else:
            return MEModelGetOneToolOutput(root=MEModelResponse(**resp_json))
