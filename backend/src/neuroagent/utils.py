...
# Load the sitemap once at module level (shared with navigate tool).
_SITEMAP_PATH = Path(__file__).parent / "sitemap-agent.json"
_SITEMAP = json.loads(_SITEMAP_PATH.read_text())

# Pre-compile route patterns: convert "[param]" segments into named regex groups.
_ROUTE_PATTERNS: list[tuple[re.Pattern[str], dict[str, Any], int]] = []
for _route_entry in _SITEMAP["routes"]:
    _route = _route_entry["route"]
    # Escape literal dots/hyphens, then replace [param] with a named group.
    _regex_parts: list[str] = []
    _dynamic_count = 0
    for _seg in _route.strip("/").split("/"):
        _m = re.match(r"^\[(.+)]$", _seg)
        if _m:
            _regex_parts.append(f"(?P<{_m.group(1)}>[^/]+)")
            _dynamic_count += 1
        else:
            _regex_parts.append(re.escape(_seg))
    _pattern = re.compile("^" + "/".join(_regex_parts) + "$")
    _ROUTE_PATTERNS.append((_pattern, _route_entry, _dynamic_count))

# Sort: fewer dynamic segments first so literal routes match before catch-alls.
_ROUTE_PATTERNS.sort(key=lambda t: t[2])
...
def extract_frontend_context(url: str) -> FrontendContextOutput:
    """Match a frontend URL against the sitemap and return a human-readable context.

    For every path parameter and search parameter present in the URL, the
    returned dict maps ``"<name>"`` → ``"<value>"``, so the output is compact
    and immediately understandable by an LLM alongside the route description.
    """
    parsed = urlparse(url)
    path = parsed.path.strip("/")
    query = parse_qs(parsed.query)

    # Try every compiled route pattern until one matches.
    for pattern, route_entry, _ in _ROUTE_PATTERNS:
