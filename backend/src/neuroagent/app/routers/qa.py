...
async def question_suggestions(
    ...
    if body.thread_id is not None:
        ...
        result = await session.execute(
            ...
            .where(Messages.thread_id == body.thread_id)
            .order_by(desc(Messages.creation_date))
            .limit(4)
        )
        db_messages = list(reversed(result.scalars().all()))

        user_ai_messages = [db_msg.parts[-1].output for db_msg in db_messages]
...
{chr(10).join(tool_info)}"""
    ...
    response = await openai_client.responses.parse(**parse_kwargs)  # type: ignore
