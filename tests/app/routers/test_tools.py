...
async def test_get_tool_calls(
    ...
    with app_client as app_client:
        ...
        message_id = messages["results"][-1]["message_id"]
        tool_calls = app_client.get(f"/tools/{thread_id}/{message_id}").json()

    assert (
        tool_calls["results"][0]
        == ToolCallSchema(
...
@pytest.mark.httpx_mock(can_send_already_matched_responses=True)
@pytest.mark.asyncio
async def test_get_tool_calls_pagination(
    patch_required_env,
    httpx_mock,
    app_client,
    db_connection,
    ...
    with app_client as app_client:
        ...
        messages = app_client.get(f"/threads/{thread_id}/messages").json()
        message_id = messages["results"][-1]["message_id"]
        tool_calls_page_1 = app_client.get(
            f"/tools/{thread_id}/{message_id}", params={"page_size": 2, "page": 1}
        ).json()
        tool_calls_page_2 = app_client.get(
            f"/tools/{thread_id}/{message_id}", params={"page_size": 2, "page": 2}
        ).json()

    assert tool_calls_page_1["page"] == 1
    assert tool_calls_page_1["page_size"] == 2
    assert tool_calls_page_1["total_pages"] == 2
    assert len(tool_calls_page_1["results"]) == 2

    assert tool_calls_page_2["page"] == 2
    assert tool_calls_page_2["page_size"] == 2
    assert tool_calls_page_2["total_pages"] == 2
    assert len(tool_calls_page_2["results"]) == 1
...
async def test_get_tool_output(
    ...
    with app_client as app_client:
        ...
        tool_call_id = tool_calls["results"][0]["tool_call_id"]
