 ...
 * 
 * Searches across 100M+ research papers using Exa AI API.
 * Provides targeted academic paper searches with deep research content coverage.
 */
...
export const LiteratureSearchOutputSchema = z.object({
  results: z.array(ArticleResultSchema),
});

export type LiteratureSearchInput = z.infer<typeof LiteratureSearchInputSchema>;
export type LiteratureSearchOutput = z.infer<typeof LiteratureSearchOutputSchema>;

/**
 * Literature Search Tool implementation
 * 
 * Searches across 100M+ research papers with full text access using Exa AI.
 * Returns detailed information about relevant academic papers including titles,
 * authors, publication dates, and full text excerpts.
 */
export class LiteratureSearchTool extends BaseTool<typeof LiteratureSearchInputSchema> {
  metadata: ToolMetadata = {
    name: 'literature-search-tool',
    nameFrontend: 'Literature Search',
    description:
  ...
  inputSchema = LiteratureSearchInputSchema;
   ...
   * @returns Search results with academic paper details and excerpts
   */
  async execute(input: LiteratureSearchInput): Promise<LiteratureSearchOutput> {
